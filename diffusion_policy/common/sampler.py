...
class SequenceSampler:
    def __init__(self,
        ...
        for i in range(len(episode_ends)):
            ...
            for current_idx in range(start_idx, end_idx, sparse_query_frequency_down_sample_steps):
        ...
        for key in lowdim_keys:
            ...
            if key.endswith('pos_abs'):
                ...
                if isinstance(axis, int):
                    axis = [axis]
                base_key = key[:-4]
                if base_key in replay_buffer:
                    self.replay_buffer[key] = replay_buffer[base_key][:, list(axis)]
                else:
                    shape = tuple(shape_meta['obs'][key]['shape'])
                    logger.warning("Missing key %s in replay buffer; filling zeros.", base_key)
                    self.replay_buffer[key] = np.zeros((replay_buffer.n_steps,) + shape, dtype=np.float32)
                ...
                if key in replay_buffer:
                    ...
                    shape = tuple(shape_meta['obs'][key]['shape'])
                    logger.warning("Missing key %s in replay buffer; filling zeros.", key)
                    self.replay_buffer[key] = np.zeros((replay_buffer.n_steps,) + shape, dtype=np.float32)
        for key in rgb_keys:
            self.replay_buffer[key] = replay_buffer[key]
